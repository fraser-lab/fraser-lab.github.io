...
#### Recently, our lab website has gone through some significant ugprades. These include moving to collections for most things instead of using the `_data` folder, moving to a CDN to load large files, and support for structure loading with [UglyMol](https://uglymol.github.io/). This guide predates these, and while it is relatively easy to copy the website still, for this guide we recommend taking a copy of one of the releases we made before incorporating these edits: https://github.com/fraser-lab/fraser-lab.github.io/releases
